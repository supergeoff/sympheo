...
mod local_tests {
    ...
    async fn test_local_git_adapter_checkout_branch() {
        ...
        adapter.checkout_branch(&repo, "feature", true).await.unwrap();
        let (branch, _) = adapter.run_git(&repo, &["rev-parse", "--abbrev-ref", "HEAD"]).await.unwrap();
    ...
    async fn test_local_git_adapter_commit() {
        ...
        let (log_out, _) = adapter.run_git(&repo, &["log", "-1", "--format=%s"]).await.unwrap();
    ...
    async fn test_local_git_adapter_status_detached() {
        let adapter = LocalGitAdapter::new();
        let repo = std::env::temp_dir().join(format!("sympheo_git_detached_{}", std::process::id()));
        let _ = tokio::fs::remove_dir_all(&repo).await;
        init_repo(&repo).await;
        commit_file(&repo, "a.txt", "hello", "first").await;

        let (hash, _) = adapter.run_git(&repo, &["rev-parse", "HEAD"]).await.unwrap();
        adapter.checkout_branch(&repo, hash.trim(), false).await.unwrap();
    ...
    async fn test_local_git_adapter_reset_hard() {
        ...
        let (first_hash, _) = adapter.run_git(&repo, &["rev-parse", "HEAD~1"]).await.unwrap();
    ...
    async fn test_local_git_adapter_merge_default() {
        ...
        adapter.merge(&repo, "feature", MergeStrategy::Default).await.unwrap();
    ...
    async fn test_local_git_adapter_fetch() {
        let adapter = LocalGitAdapter::new();
        let remote = std::env::temp_dir().join(format!("sympheo_git_remote_{}", std::process::id()));
    ...
    async fn test_local_git_adapter_push_failure() {
        let adapter = LocalGitAdapter::new();
        let repo = std::env::temp_dir().join(format!("sympheo_git_push_bad_{}", std::process::id()));
        let _ = tokio::fs::remove_dir_all(&repo).await;
        init_repo(&repo).await;
    ...
    async fn test_local_git_adapter_merge_failure() {
        let adapter = LocalGitAdapter::new();
        let repo = std::env::temp_dir().join(format!("sympheo_git_merge_bad_{}", std::process::id()));
        let _ = tokio::fs::remove_dir_all(&repo).await;
        init_repo(&repo).await;
        let result = adapter.merge(&repo, "nonexistent", MergeStrategy::Default).await;
        assert!(result.is_err());
        let _ = tokio::fs::remove_dir_all(&repo).await;
    }
    ...
    async fn test_local_git_adapter_merge_ours() {
        let adapter = LocalGitAdapter::new();
        let repo = std::env::temp_dir().join(format!("sympheo_git_merge_ours_{}", std::process::id()));
        let _ = tokio::fs::remove_dir_all(&repo).await;
        init_repo(&repo).await;
        commit_file(&repo, "a.txt", "hello", "first").await;
        ...
        tokio::fs::write(repo.join("a.txt"), "main text").await.unwrap();
        let _ = tokio::process::Command::new("git")
            .arg("-C").arg(&repo)
            .args(&["add", "a.txt"])
            .output().await.unwrap();
        ...
        adapter.merge(&repo, "feature", MergeStrategy::Ours).await.unwrap();
    ...
    fn test_local_git_adapter_default() {
        let adapter = LocalGitAdapter::default();
        let _ = adapter;
    }
    ...
    async fn test_local_git_adapter_push_success() {
        let adapter = LocalGitAdapter::new();
        let remote = std::env::temp_dir().join(format!("sympheo_git_push_remote_{}", std::process::id()));
        let local = std::env::temp_dir().join(format!("sympheo_git_push_local_{}", std::process::id()));
        let _ = tokio::fs::remove_dir_all(&remote).await;
        let _ = tokio::fs::remove_dir_all(&local).await;
        ...
        init_repo(&local).await;
        tokio::fs::write(local.join("a.txt"), "hello").await.unwrap();
        let _ = tokio::process::Command::new("git")
            .arg("-C").arg(&local)
            .args(&["add", "a.txt"])
            .output().await.unwrap();
    ...
    async fn test_local_git_adapter_merge_theirs() {
        let adapter = LocalGitAdapter::new();
        let repo = std::env::temp_dir().join(format!("sympheo_git_merge_theirs_{}", std::process::id()));
        let _ = tokio::fs::remove_dir_all(&repo).await;
        init_repo(&repo).await;
        commit_file(&repo, "a.txt", "hello", "first").await;
        ...
        adapter.merge(&repo, "feature", MergeStrategy::Theirs).await.unwrap();
